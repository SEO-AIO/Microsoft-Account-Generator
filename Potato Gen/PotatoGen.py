...
def setup():
    ctypes.windll.kernel32.SetConsoleTitleW("PotatoGen v1")
    os.system("cls")
    os.system("color 03")
    print("""
...
class PotatoGen:
    ...
    def credentials(self):
        ...
        while True:
            try:
                self.NAME_INPUT = self.WORDS.get_random_word(hasDictionaryDef="true", minLength=6, maxLength=6).title()
                self.LASTNAME_INPUT = self.WORDS.get_random_word(hasDictionaryDef="true", minLength=6, maxLength=6).title()
                break
            except (Exception,):
                sleep(2)
                continue
